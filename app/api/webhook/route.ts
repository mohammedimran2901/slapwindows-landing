...
import { createLicense, findLicenseByPaymentId } from "@/models/License";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const existing = await findLicenseByPaymentId(orderId);
      if (existing) {
      ...
      // Send email (only if Resend key is configured)
      ...
      if (resendKey) {
        ...
        console.log(`[Webhook] No Resend key — email not sent. License: ${key}`);
      }
