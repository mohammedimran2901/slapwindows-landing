...
function SuccessContent() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center text-center px-8">
      <div>
        {status === "loading" && (
          <p className="text-[#888]">Processing your order...</p>
        )}

        {status === "done" && (
          <div>
            <div className="text-[64px] mb-4">🎉</div>
            <h1 className="text-[2.5rem] font-bold tracking-[-1px] mb-2">Payment Successful!</h1>
            ...
            <a href={downloadUrl} className="inline-block bg-[#e63535] text-white px-8 py-3.5 rounded-xl text-base font-medium no-underline hover:opacity-90 transition">
              Download SlapWindows.exe
            </a>
            <p className="text-xs text-[#555] mt-6">
              Problem?{" "}
              <a href="https://x.com/amitkushh" className="text-[#888]">x.com/amitkushh</a>
            </p>
          </div>
        )}
        ...
        {status === "error" && (
          <div>
            ...
            <p className="text-[#888]">message me on x.com/amitkushh</p>
          </div>
        )}
      </div>
    </main>
